...
use App\MongoDB\Models\Doc\ScaleTopicDoc;
use App\MongoDB\Models\ScaleTopic;
use Illuminate\Support\Facades\DB;

//该版本Manager主要支持MongoDB的数据模型
class ScaleTopicManager
{
    ...
    */
    public static function getById($_id)
    {
        ...
        $info = ScaleTopic::where('_id', $_id)->first();
        return $info;
    }
    ...
    */
    ...
    {
        $info = ScaleTopic::withTrashed()->where('_id', $_id)->first();
        return $info;
    }
    ...
    */
    public static function getListByCon($con_arr, $is_paginate, $page = 1)
    {
        ...
        if (array_key_exists('topic_no', $con_arr) && !Utils::isObjNull($con_arr['topic_no'])) {
        $infos = $infos->where('topic_no', '=', $con_arr['topic_no']);
        }
        ...
        if (array_key_exists('body_parts', $con_arr) && !Utils::isObjNull($con_arr['body_parts'])) {
        $infos = $infos->where('body_parts', '=', $con_arr['body_parts']);
        }
        ...
        if (array_key_exists('is_test', $con_arr) && !Utils::isObjNull($con_arr['is_test'])) {
        $infos = $infos->where('is_test', '=', $con_arr['is_test']);
        }
...
{
    $ref = new \ReflectionClass(ScaleTopicDoc::class);
    ...
    $info->scale_id = $data['scale_id'];
            ...
            if (array_key_exists('topic_no', $data)) {
    $info->topic_no = $data['topic_no'];
    }
    ...
    $info->intro = $data['intro'];
            ...
            if (array_key_exists('body_parts', $data)) {
    $info->body_parts = $data['body_parts'];
    }
