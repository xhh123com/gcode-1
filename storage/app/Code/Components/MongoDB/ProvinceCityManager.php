...
use App\MongoDB\Models\Doc\ProvinceCityDoc;
use App\MongoDB\Models\ProvinceCity;
...
class ProvinceCityManager
{
    ...
    */
    public static function getById($_id)
    {
        ...
        $info = ProvinceCity::where('_id', $_id)->first();
        return $info;
    }
    ...
    */
    ...
    {
        $info = ProvinceCity::withTrashed()->where('_id', $_id)->first();
        return $info;
    }
    ...
    */
    public static function getListByCon($con_arr, $is_paginate, $page = 1)
    {
        ...
        if (array_key_exists('parentId', $con_arr) && !Utils::isObjNull($con_arr['parentId'])) {
        $infos = $infos->where('parentId', '=', $con_arr['parentId']);
        }
    
        if (array_key_exists('cityCode', $con_arr) && !Utils::isObjNull($con_arr['cityCode'])) {
        $infos = $infos->where('cityCode', '=', $con_arr['cityCode']);
        }
...
{
    $ref = new \ReflectionClass(ProvinceCityDoc::class);
