...
use App\MongoDB\Models\Doc\MaterialRecordDoc;
use App\MongoDB\Models\MaterialRecord;
...
class MaterialRecordManager
{
    ...
    */
    public static function getById($_id)
    {
        ...
        $info = MaterialRecord::where('_id', $_id)->first();
        return $info;
    }
    ...
    */
    ...
    {
        $info = MaterialRecord::withTrashed()->where('_id', $_id)->first();
        return $info;
    }
    ...
    */
    public static function getListByCon($con_arr, $is_paginate, $page = 1)
    {
        ...
        if (array_key_exists('material_type', $con_arr) && !Utils::isObjNull($con_arr['material_type'])) {
        $infos = $infos->where('material_type', '=', $con_arr['material_type']);
        }
    
        if (array_key_exists('material_url', $con_arr) && !Utils::isObjNull($con_arr['material_url'])) {
        $infos = $infos->where('material_url', '=', $con_arr['material_url']);
        }
...
{
    $ref = new \ReflectionClass(MaterialRecordDoc::class);
    ...
    $info->material_type = $data['material_type'];
    }
        
    
            if (array_key_exists('material_url', $data)) {
