...
use App\MongoDB\Models\Doc\CollectionPatientDoc;
use App\MongoDB\Models\CollectionPatient;
use Illuminate\Support\Facades\DB;

//该版本Manager主要支持MongoDB的数据模型
class CollectionPatientManager
{
    ...
    */
    public static function getById($_id)
    {
        ...
        $info = CollectionPatient::where('_id', $_id)->first();
        return $info;
    }
    ...
    */
    ...
    {
        $info = CollectionPatient::withTrashed()->where('_id', $_id)->first();
        return $info;
    }
    ...
    */
    public static function getListByCon($con_arr, $is_paginate, $page = 1)
    {
        ...
        if (array_key_exists('patient_tw_id', $con_arr) && !Utils::isObjNull($con_arr['patient_tw_id'])) {
        $infos = $infos->where('patient_tw_id', '=', $con_arr['patient_tw_id']);
        }
    
        if (array_key_exists('circle_text', $con_arr) && !Utils::isObjNull($con_arr['circle_text'])) {
        $infos = $infos->where('circle_text', '=', $con_arr['circle_text']);
        }
    
        if (array_key_exists('circle_imgs', $con_arr) && !Utils::isObjNull($con_arr['circle_imgs'])) {
        $infos = $infos->where('circle_imgs', '=', $con_arr['circle_imgs']);
        }
...
{
    $ref = new \ReflectionClass(CollectionPatientDoc::class);
    ...
    $info->img = $data['img'];
            ...
            if (array_key_exists('patient_tw_id', $data)) {
    $info->patient_tw_id = $data['patient_tw_id'];
    }
        
    
            if (array_key_exists('circle_text', $data)) {
    $info->circle_text = $data['circle_text'];
    }
        
    
            if (array_key_exists('circle_imgs', $data)) {
    $info->circle_imgs = $data['circle_imgs'];
    }
