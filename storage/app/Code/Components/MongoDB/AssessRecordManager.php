...
use App\MongoDB\Models\Doc\AssessRecordDoc;
use App\MongoDB\Models\AssessRecord;
use Illuminate\Support\Facades\DB;

//该版本Manager主要支持MongoDB的数据模型
class AssessRecordManager
{
    ...
    */
    public static function getById($_id)
    {
        ...
        $info = AssessRecord::where('_id', $_id)->first();
        return $info;
    }
    ...
    */
    ...
    {
        $info = AssessRecord::withTrashed()->where('_id', $_id)->first();
        return $info;
    }
    ...
    */
    public static function getListByCon($con_arr, $is_paginate, $page = 1)
    {
        ...
        if (array_key_exists('scale_id', $con_arr) && !Utils::isObjNull($con_arr['scale_id'])) {
        $infos = $infos->where('scale_id', '=', $con_arr['scale_id']);
        }
        ...
        if (array_key_exists('assess_at', $con_arr) && !Utils::isObjNull($con_arr['assess_at'])) {
        $infos = $infos->where('assess_at', '=', $con_arr['assess_at']);
        }
        ...
        if (array_key_exists('test_scale_json', $con_arr) && !Utils::isObjNull($con_arr['test_scale_json'])) {
        $infos = $infos->where('test_scale_json', '=', $con_arr['test_scale_json']);
        }
    
        if (array_key_exists('no_test_scale_json', $con_arr) && !Utils::isObjNull($con_arr['no_test_scale_json'])) {
        $infos = $infos->where('no_test_scale_json', '=', $con_arr['no_test_scale_json']);
        }
    
        if (array_key_exists('more_scale_json', $con_arr) && !Utils::isObjNull($con_arr['more_scale_json'])) {
        $infos = $infos->where('more_scale_json', '=', $con_arr['more_scale_json']);
        }
    
        if (array_key_exists('video_annex', $con_arr) && !Utils::isObjNull($con_arr['video_annex'])) {
        $infos = $infos->where('video_annex', '=', $con_arr['video_annex']);
        }
    
        if (array_key_exists('other_annex', $con_arr) && !Utils::isObjNull($con_arr['other_annex'])) {
        $infos = $infos->where('other_annex', '=', $con_arr['other_annex']);
        }
    
        if (array_key_exists('total_score', $con_arr) && !Utils::isObjNull($con_arr['total_score'])) {
        $infos = $infos->where('total_score', '=', $con_arr['total_score']);
        }
...
{
    $ref = new \ReflectionClass(AssessRecordDoc::class);
    ...
    $info->is_share = $data['is_share'];
            ...
            if (array_key_exists('scale_id', $data)) {
    $info->scale_id = $data['scale_id'];
    }
    ...
    $info->hospital_id = $data['hospital_id'];
            ...
            if (array_key_exists('assess_at', $data)) {
    $info->assess_at = $data['assess_at'];
    }
    ...
    $info->intro = $data['intro'];
            ...
            if (array_key_exists('test_scale_json', $data)) {
    $info->test_scale_json = $data['test_scale_json'];
    }
        
    
            if (array_key_exists('no_test_scale_json', $data)) {
    $info->no_test_scale_json = $data['no_test_scale_json'];
    }
        
    
            if (array_key_exists('more_scale_json', $data)) {
    $info->more_scale_json = $data['more_scale_json'];
    }
        
    
            if (array_key_exists('video_annex', $data)) {
    $info->video_annex = $data['video_annex'];
    }
        
    
            if (array_key_exists('other_annex', $data)) {
    $info->other_annex = $data['other_annex'];
    }
        
    
            if (array_key_exists('total_score', $data)) {
    $info->total_score = $data['total_score'];
    }
