...
use App\MongoDB\Models\Doc\ScaleAnswerDoc;
use App\MongoDB\Models\ScaleAnswer;
use Illuminate\Support\Facades\DB;

//该版本Manager主要支持MongoDB的数据模型
class ScaleAnswerManager
{
    ...
    */
    public static function getById($_id)
    {
        ...
        $info = ScaleAnswer::where('_id', $_id)->first();
        return $info;
    }
    ...
    */
    ...
    {
        $info = ScaleAnswer::withTrashed()->where('_id', $_id)->first();
        return $info;
    }
    ...
    */
    public static function getListByCon($con_arr, $is_paginate, $page = 1)
    {
        ...
        if (array_key_exists('scale_topic_id', $con_arr) && !Utils::isObjNull($con_arr['scale_topic_id'])) {
        $infos = $infos->where('scale_topic_id', '=', $con_arr['scale_topic_id']);
        }
...
{
    $ref = new \ReflectionClass(ScaleAnswerDoc::class);
    ...
    $info->admin_id = $data['admin_id'];
            ...
            if (array_key_exists('scale_topic_id', $data)) {
    $info->scale_topic_id = $data['scale_topic_id'];
    }
