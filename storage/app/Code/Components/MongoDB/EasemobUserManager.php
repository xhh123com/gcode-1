...
use App\MongoDB\Models\Doc\EasemobUserDoc;
use App\MongoDB\Models\EasemobUser;
...
class EasemobUserManager
{
    ...
    */
    public static function getById($_id)
    {
        ...
        $info = EasemobUser::where('_id', $_id)->first();
        return $info;
    }
    ...
    */
    ...
    {
        $info = EasemobUser::withTrashed()->where('_id', $_id)->first();
        return $info;
    }
    ...
    */
    public static function getListByCon($con_arr, $is_paginate, $page = 1)
    {
        ...
        if (array_key_exists('uuid', $con_arr) && !Utils::isObjNull($con_arr['uuid'])) {
        $infos = $infos->where('uuid', '=', $con_arr['uuid']);
        }
    
        if (array_key_exists('username', $con_arr) && !Utils::isObjNull($con_arr['username'])) {
        $infos = $infos->where('username', '=', $con_arr['username']);
        }
        ...
        if (array_key_exists('nickname', $con_arr) && !Utils::isObjNull($con_arr['nickname'])) {
        $infos = $infos->where('nickname', '=', $con_arr['nickname']);
        }
    
        if (array_key_exists('activated', $con_arr) && !Utils::isObjNull($con_arr['activated'])) {
        $infos = $infos->where('activated', '=', $con_arr['activated']);
        }
...
{
    $ref = new \ReflectionClass(EasemobUserDoc::class);
