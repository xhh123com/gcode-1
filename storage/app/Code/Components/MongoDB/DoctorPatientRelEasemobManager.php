...
use App\MongoDB\Models\Doc\DoctorPatientRelEasemobDoc;
use App\MongoDB\Models\DoctorPatientRelEasemob;
use Illuminate\Support\Facades\DB;

//该版本Manager主要支持MongoDB的数据模型
class DoctorPatientRelEasemobManager
{
    ...
    */
    public static function getById($_id)
    {
        ...
        $info = DoctorPatientRelEasemob::where('_id', $_id)->first();
        return $info;
    }
    ...
    */
    ...
    {
        $info = DoctorPatientRelEasemob::withTrashed()->where('_id', $_id)->first();
        return $info;
    }
    ...
    */
    ...
    {
        $infos = new DoctorPatientRelEasemob();
        ...
        if (array_key_exists('owner_username', $con_arr) && !Utils::isObjNull($con_arr['owner_username'])) {
        $infos = $infos->where('owner_username', '=', $con_arr['owner_username']);
        }
    
        if (array_key_exists('friend_username', $con_arr) && !Utils::isObjNull($con_arr['friend_username'])) {
        $infos = $infos->where('friend_username', '=', $con_arr['friend_username']);
        }
...
{
    $ref = new \ReflectionClass(DoctorPatientRelEasemobDoc::class);
    ...
    $info->collection_name = $data['collection_name'];
            ...
            if (array_key_exists('owner_username', $data)) {
    $info->owner_username = $data['owner_username'];
    }
        
    
            if (array_key_exists('friend_username', $data)) {
    $info->friend_username = $data['friend_username'];
    }
