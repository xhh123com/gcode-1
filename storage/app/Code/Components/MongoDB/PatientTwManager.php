...
use App\MongoDB\Models\Doc\PatientTwDoc;
use App\MongoDB\Models\PatientTw;
use Illuminate\Support\Facades\DB;

//该版本Manager主要支持MongoDB的数据模型
class PatientTwManager
{
    ...
    */
    public static function getById($_id)
    {
        ...
        $info = PatientTw::where('_id', $_id)->first();
        return $info;
    }
    ...
    */
    ...
    {
        $info = PatientTw::withTrashed()->where('_id', $_id)->first();
        return $info;
    }
    ...
    */
    public static function getListByCon($con_arr, $is_paginate, $page = 1)
    {
        ...
        if (array_key_exists('create_date', $con_arr) && !Utils::isObjNull($con_arr['create_date'])) {
        $infos = $infos->where('create_date', '=', $con_arr['create_date']);
        }
...
{
    $ref = new \ReflectionClass(PatientTwDoc::class);
