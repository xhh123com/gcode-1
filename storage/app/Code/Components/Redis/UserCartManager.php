...
class UserCartManager
{
    ...
    * 2020-02-02 04:07:18
    ...
    {
        $info = UserCart::where('id', $id)->first();
        return $info;
    }
    ...
    */
    ...
    {
        $info = UserCart::withTrashed()->where('id', $id)->first();
        return $info;
    }
    ...
    */
    public static function deleteById($id)
    {
        ...
        $result = $info->delete();
        RedisManager::del("user_cart:" . $info->id);
        return $result;
    }
    ...
    * 2020-02-02 04:07:18
    ...
    public static function getListByCon($con_arr, $is_paginate)
    {
        ...
        if (array_key_exists('product_sku_id', $con_arr) && !Utils::isObjNull($con_arr['product_sku_id'])) {
            $infos = $infos->where('product_sku_id', '=', $con_arr['product_sku_id']);
        }
    
        if (array_key_exists('amount', $con_arr) && !Utils::isObjNull($con_arr['amount'])) {
            $infos = $infos->where('amount', '=', $con_arr['amount']);
        }
    
        if (array_key_exists('is_select', $con_arr) && !Utils::isObjNull($con_arr['is_select'])) {
            $infos = $infos->where('is_select', '=', $con_arr['is_select']);
        }
    ...
    * 2020-02-02 04:07:18
    ...
    public static function setInfo($info, $data,$level=0)
    {
        ...
        if (array_key_exists('product_sku_id', $data)) {
                $info->product_sku_id = $data['product_sku_id'];
            }
        ...
        if (array_key_exists('is_select', $data)) {
                $info->is_select = $data['is_select'];
            }
